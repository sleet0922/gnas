...
import { apiGet, apiPost, type LoginStatus } from '@/composables/useApi'
...
onMounted(async () => {
  const data = await apiGet<LoginStatus>('/api/login')
  if (data) {
...
async function login() {
  ...
  loading.value = true
  try {
    const res = await apiPost<{ token: string }>('/api/login', {
